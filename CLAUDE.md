...
### Edge-fade shader
`shaders/truck_fade.gdshader` (a `CanvasItem` shader on the truck body) fades the truck at monitor edges. It needs **global** screen coordinates, so `truck_window._process()` feeds it `window_x`, `window_width`, `screen_left`, `screen_right`, and `fade_margin` (from config) every frame via `truck_entity.update_shader_parameters()`. With multiple windows, each `TruckWindow` feeds its own monitor's bounds — this only works correctly because each entity owns a duplicated material (see above).
