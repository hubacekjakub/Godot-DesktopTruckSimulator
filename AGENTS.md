...
*   **Autoloads (Singletons)**:
    ...
    *   `WindowManager` (`autoloads/window_manager.gd`): Manages OS Window instantiation, bounds, and visibility.
...
*   **Godot Bug #71642 Workaround**: Windows ignores transparency flags set before native window handles are fully created.
    *   *Workaround*: Start window off-screen at `(-10000, -10000)` with `transparent = false`. Wait 2 frames via `await get_tree().process_frame`, toggle to `true`, and set `DisplayServer` transparency flags before moving on-screen using `_is_initialized` guard.
